...
class _FakeAdminListingsRepository implements AdminListingsRepository {
  ...
  Future<AdminListingsPageResult> fetchPage({
    ...
    return AdminListingsPageResult(items: items, cursor: null, hasMore: false);
  }
}

void main() {
  testWidgets(
    'conserve les échecs sélectionnés après une suppression partielle',
    (tester) async {
      Map<String, Object?>? sentPayload;
      final repository = _FakeAdminListingsRepository(<AdminListingRecord>[
      ...
      await tester.tap(
        find.byTooltip('Sélectionner toutes les annonces visibles'),
      );
      await tester.pump();
