...
class FastLanePlannerTest(unittest.TestCase):
    def test_ranks_testable_business_logic_above_hard_firebase_code(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            lcov.write_text(
                "\n".join(
                    [
                        "SF:lib/models/order.dart",
                        "LF:40",
                        "LH:10",
                        "DA:1,0",
                        "end_of_record",
                        "SF:lib/services/firestore_service.dart",
                        "LF:70",
                        "LH:10",
                        "DA:1,0",
                        "end_of_record",
                        "SF:lib/widgets/payment_card.dart",
                        "LF:35",
                        "LH:5",
                        "DA:1,0",
                        "end_of_record",
            ...
            model = next(
                item for item in candidates if item.path.endswith("order.dart")
            )
            firebase = next(
                item
                for item in candidates
                if item.path.endswith("firestore_service.dart")
            )
            self.assertGreater(model.testability, firebase.testability)
            self.assertGreater(model.score, 0)

    def test_builds_at_most_two_non_overlapping_high_yield_packs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for path in (
                "lib/models/a.dart",
            ...
            plan = plan_to_dict(hit, found, candidates, packs)
            self.assertEqual(plan["policy"]["max_lanes"], 2)

    def test_gain_threshold_only_drops_near_the_target(self) -> None:
