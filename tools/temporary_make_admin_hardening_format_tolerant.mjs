...
const replacement = `async function patchAdminFetchOnce() {
  ...
  for (const target of targets) {
    ...
    const fetchOncePattern = new RegExp(
      \\`${'${escapedTarget}'}\\\\s*=\\\\s*FirebaseFirestore\\\\.instance[\\\\s\\\\S]{0,500}?\\\\.get\\\\(\\\\)\\\\s*\\\\.asStream\\\\(\\\\);\\`,
    );
    if (fetchOncePattern.test(content)) continue;

    const realtimePattern = new RegExp(
      \\`(${'${escapedTarget}'}\\\\s*=\\\\s*FirebaseFirestore\\\\.instance[\\\\s\\\\S]{0,500}?)\\\\.snapshots\\\\(\\\\);\\`,
    );
    const matches = [...content.matchAll(new RegExp(realtimePattern.source, 'g'))];
    if (matches.length !== 1) {
      throw new Error(
        \\`admin fetch once ${'${target}'}: expected one snapshots query or an existing get().asStream(), found ${'${matches.length}'}\\`,
      );
    }
