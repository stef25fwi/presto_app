...
const registryPath = path.join(root, 'quality/production_go_live_readiness.json');
const reportPath = path.join(root, 'build/quality/production-go-live-readiness-report.json');
...
for (const control of registry.controls ?? []) {
  ...
  if (enforce && control.status !== 'verified') {
    failures.push(`${control.id}: statut ${control.status}`);
  }
}
