...
exports.onLegalPrivacySettingsUpdated = exports.onLegalTermsSettingsUpdated = void 0;
...
async function emitPrivacyUpdatedEvents(version, effectiveDate, privacyUrl) {
    let lastDocId;
    while (true) {
        let query = firestore_2.db.collection(constants_1.COLLECTIONS.users).orderBy("__name__").limit(200);
        if (lastDocId) {
        ...
        for (const doc of snap.docs) {
            ...
            await firestore_2.db.collection(constants_1.COLLECTIONS.emailEvents).doc(eventId).set({
                ...
                recipient_user_id: doc.id,
                dedupe_key: (0, hash_1.sha256)(`legal.privacy.updated:${version}:${doc.id}`),
                occurred_at: Date.now(),
                payload: {
                    recipient_email: recipientEmail,
                    firstName: String(user.displayName || user.display_name || "").split(" ")[0] || "",
...
exports.onLegalPrivacySettingsUpdated = (0, firestore_1.onDocumentUpdated)(`${constants_1.COLLECTIONS.systemSettings}/legal_privacy`, async (event) => {
    const before = (event.data?.before.data() ?? {});
    const after = (event.data?.after.data() ?? {});
    const nextVersion = String(after.current_version || "").trim();
    const previousVersion = String(before.current_version || "").trim();
