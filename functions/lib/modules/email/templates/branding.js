...
exports.applyFirestoreEmailBranding = applyFirestoreEmailBranding;
const EMAIL_BRANDING_MARKER = "data-presto-email-branding";
function buildBrandingBlock(preheader) {
    const safePreheader = preheader.trim();
    return (`<div ${EMAIL_BRANDING_MARKER}="true" style="padding:0 0 24px 0">` +
...
function buildBrandedDocument(bodyHtml, preheader) {
...
function applyFirestoreEmailBranding(templateHtml, preheader) {
    const normalizedHtml = templateHtml.trim();
    if (!normalizedHtml) {
        return buildBrandedDocument("", preheader);
    }
    if (normalizedHtml.includes(EMAIL_BRANDING_MARKER) ||
        normalizedHtml.includes("{{brandLogoUrl}}") ||
        /logowebp\.webp/i.test(normalizedHtml)) {
        return normalizedHtml;
    }
    if (/<body[^>]*>/i.test(normalizedHtml)) {
        return normalizedHtml.replace(/<body([^>]*)>/i, `<body$1>${buildBrandingBlock(preheader)}`);
    }
