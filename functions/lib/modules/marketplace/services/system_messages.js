...
exports.sendTeamBroadcastMessage = sendTeamBroadcastMessage;
...
function teamConversationId(userId) {
    return `team_ilipresto_${userId.replaceAll("/", "_")}`;
}
async function sendTeamBroadcastMessage({ userId, messageId, body, campaignTitle, campaignUrl, }) {
    const normalizedUserId = normalizeString(userId);
    const normalizedBody = normalizeString(body);
    if (!normalizedUserId || !normalizedBody)
        return "";
    ...
    await firestore_1.db.runTransaction(async (transaction) => {
        ...
        transaction.set(convRef, (0, mirror_1.buildConversationMirrorFields)({
            ...
            unreadCount: {
                ...(existing?.unreadCount ?? {}),
                [normalizedUserId]: firebase_admin_1.default.firestore.FieldValue.increment(1),
                [SYSTEM_SENDER_ID]: 0,
            },
            createdAt: existing?.createdAt ?? firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
            updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
            ...
            lastSenderId: SYSTEM_SENDER_ID,
            lastSenderName: SYSTEM_SENDER_NAME,
            messageCount: convSnap.exists ? firebase_admin_1.default.firestore.FieldValue.increment(1) : 1,
        }), { merge: true });
        transaction.set(messageRef, {
            text: normalizedBody,
            body: normalizedBody,
    ...
    await Promise.all([
        (0, counters_1.refreshUnreadMessageCount)(normalizedUserId),
