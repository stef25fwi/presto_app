...
exports.publishMaturedReviewsV2 = exports.replyToReviewV2 = exports.reportReviewV2 = exports.getUserTrustScoreV2 = exports.submitMutualVerifiedReview = exports.ratingsV2Enabled = void 0;
...
function readRole(value, fallback) {
    const role = normalizeString(value);
    return role === "requester" || role === "provider" ? role : fallback;
}
function assertRole(value, fieldName) {
    const role = normalizeString(value);
    if (role === "requester" || role === "provider")
        return role;
    throw new https_1.HttpsError("invalid-argument", `${fieldName} must be requester or provider`);
}
...
function readCriteria(rawData, reviewedRole) {
...
function averageCriteria(criteria) {
...
async function loadListing(offerId) {
...
async function findConversationBetween(offerId, userA, userB) {
    ...
    for (const snapshot of snapshots) {
        for (const doc of snapshot.docs) {
            ...
            if (participants.includes(userA) && participants.includes(userB))
                return doc.id;
        }
    }
    throw new https_1.HttpsError("permission-denied", "No verified conversation found for this review");
}
async function assertReviewEligibility({ offerId, reviewerId, reviewedUserId, reviewerRole, reviewedRole, }) {
    if (reviewerId === reviewedUserId)
        throw new https_1.HttpsError("failed-precondition", "You cannot review yourself");
    if (reviewerRole === reviewedRole)
        throw new https_1.HttpsError("invalid-argument", "Reviews must be reciprocal between requester and provider roles");
    const listingData = await loadListing(offerId);
    const ownerId = readListingOwnerId(listingData);
    if (!ownerId)
        throw new https_1.HttpsError("failed-precondition", "Listing owner is missing");
    if (reviewerRole === "requester" && reviewerId !== ownerId)
        throw new https_1.HttpsError("permission-denied", "Only the listing requester can review the provider as requester");
    if (reviewedRole === "requester" && reviewedUserId !== ownerId)
        throw new https_1.HttpsError("permission-denied", "Only the listing requester can be reviewed as requester");
...
function isReviewPublicForScore(data, nowMs = Date.now()) {
    if (data.visibleOnProfile === false || data.isVerified !== true)
        return false;
    const status = normalizeString(data.status);
    if (status === "published")
        return true;
    if (status !== "pending_peer_review")
        return false;
    const publishAfterMs = readTimestampMillis(data.publishAfter);
    return publishAfterMs != null && publishAfterMs <= nowMs;
}
function readReviewForScore(doc) {
    const data = (doc.data() ?? {});
    ...
    const criteria = Object.fromEntries(Object.entries(criteriaSource)
        .map(([key, value]) => [
        key,
        Number(value || 0),
    ])
...
function summarizeRole(reviews, role) {
...
function summarizeTrustScoreV2(reviews, userData) {
    ...
    const lastReviewAtMillis = reviews.map((review) => review.publishedAtMillis ?? review.createdAtMillis).filter((value) => typeof value === "number").sort((a, b) => b - a)[0] ?? null;
    return {
        provider,
        requester,
        global: {
            ...
            score100,
            profileVerified,
            lastReviewAt: lastReviewAtMillis == null ? null : firebase_admin_1.default.firestore.Timestamp.fromMillis(lastReviewAtMillis),
        },
    };
}
function legacyTrustScoreFromV2(scoreV2) {
    const allCriteria = { ...scoreV2.provider.criteriaAverages, ...scoreV2.requester.criteriaAverages };
...
async function recalculateUserTrustScoreV2(userId) {
    const [userSnap, reviewsSnap] = await Promise.all([
        firestore_1.db.collection(constants_1.COLLECTIONS.users).doc(userId).get(),
        firestore_1.db.collection(REVIEWS_COLLECTION).where("reviewedUserId", "==", userId).get(),
    ]);
    const reviews = [];
    ...
    for (const doc of reviewsSnap.docs) {
        const data = (doc.data() ?? {});
        const status = normalizeString(data.status);
        if (status === "pending_peer_review" || status === "pending_moderation")
            pendingReviewsCount += 1;
        if (!isReviewPublicForScore(data))
            continue;
    ...
    await firestore_1.db.collection(constants_1.COLLECTIONS.users).doc(userId).set({ trustScore, trustScoreV2, updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() }, { merge: true });
    return { trustScore, trustScoreV2 };
}
async function publishReciprocalReviewsIfReady(offerId, reviewerId, reviewedUserId) {
    const counterpartSnap = await firestore_1.db.collection(REVIEWS_COLLECTION).where("offerId", "==", offerId).where("reviewerId", "==", reviewedUserId).where("reviewedUserId", "==", reviewerId).limit(5).get();
    const reviewIds = counterpartSnap.docs.filter((doc) => normalizeString(doc.data().status) === "pending_peer_review").map((doc) => doc.id);
    if (reviewIds.length === 0)
        return;
    const currentSnap = await firestore_1.db.collection(REVIEWS_COLLECTION).where("offerId", "==", offerId).where("reviewerId", "==", reviewerId).where("reviewedUserId", "==", reviewedUserId).limit(5).get();
    for (const doc of currentSnap.docs)
        if (normalizeString(doc.data().status) === "pending_peer_review")
            reviewIds.push(doc.id);
    const batch = firestore_1.db.batch();
    for (const reviewId of Array.from(new Set(reviewIds))) {
        batch.set(firestore_1.db.collection(REVIEWS_COLLECTION).doc(reviewId), { status: "published", visibleOnProfile: true, publishedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(), updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() }, { merge: true });
    }
    await batch.commit();
}
function requesterFoundProviderUpdate(reviewedUserId, reason) {
...
exports.submitMutualVerifiedReview = (0, https_1.onCall)({ region: env_1.PROJECT_REGION, enforceAppCheck: env_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    const rateAllowed = await (0, rate_limit_1.canProceedRateLimited)("verified_review_v2_submit", reviewerId, 20, 24 * 60 * 60 * 1000);
    if (!rateAllowed)
    ...
    try {
        ...
        const now = firebase_admin_1.default.firestore.FieldValue.serverTimestamp();
        const publishAfter = firebase_admin_1.default.firestore.Timestamp.fromMillis(Date.now() + REVIEW_WINDOW_DAYS * 24 * 60 * 60 * 1000);
        const closeReason = "J’ai trouvé quelqu’un sur iliprestō";
        await firestore_1.db.runTransaction(async (transaction) => {
            const reviewSnap = await transaction.get(reviewRef);
            if (reviewSnap.exists)
                throw new https_1.HttpsError("already-exists", "A review already exists for this listing, user and role");
        ...
        logger_1.logger.info("marketplace_mutual_review_submitted", { offerId, reviewId, reviewerId, reviewedUserId, reviewerRole, reviewedRole, status: moderation.status });
        ...
        throw (0, errors_1.toHttpsError)(error, "Unable to submit mutual verified review");
    }
});
exports.getUserTrustScoreV2 = (0, https_1.onCall)({ region: env_1.PROJECT_REGION, enforceAppCheck: env_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    try {
        ...
        return { ok: true, ratingsV2Enabled: exports.ratingsV2Enabled, ratingsPaidShowcaseEnabled: false, trustScore, trustScoreV2, latestReviews };
    }
    catch (error) {
        throw (0, errors_1.toHttpsError)(error, "Unable to load trust score v2");
    }
});
exports.reportReviewV2 = (0, https_1.onCall)({ region: env_1.PROJECT_REGION, enforceAppCheck: env_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    try {
        ...
        await firestore_1.db.runTransaction(async (transaction) => {
            ...
            if (reviewedUserId !== reporterId && reviewerId !== reporterId)
                throw new https_1.HttpsError("permission-denied", "Only review participants can report this review");
            transaction.set(reportRef, { reviewId, reportedBy: reporterId, reason, details: details || null, createdAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(), status: "pending", ratingSchemaVersion: 2 });
            transaction.set(reviewRef, { reportCount: firebase_admin_1.default.firestore.FieldValue.increment(1), status: "disputed", visibleOnProfile: false, disputeCount: firebase_admin_1.default.firestore.FieldValue.increment(1), updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() }, { merge: true });
        });
        if (reviewedUserId)
            await recalculateUserTrustScoreV2(reviewedUserId);
        return { ok: true, statusChanged: true };
    }
    catch (error) {
        throw (0, errors_1.toHttpsError)(error, "Unable to report review v2");
    }
});
exports.replyToReviewV2 = (0, https_1.onCall)({ region: env_1.PROJECT_REGION, enforceAppCheck: env_1.ENFORCE_APP_CHECK }, async (request) => {
    ...
    try {
        ...
        await replyRef.set({ reviewId, reviewedUserId: userId, replyText: moderation.comment, createdAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(), status: replyStatus, moderationFlags: moderation.flags, ratingSchemaVersion: 2 }, { merge: true });
        await firestore_1.db.collection(REVIEWS_COLLECTION).doc(reviewId).set({ hasReply: true, replyText: moderation.comment, replyStatus, updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() }, { merge: true });
        return { ok: true, status: replyStatus };
    }
    catch (error) {
        throw (0, errors_1.toHttpsError)(error, "Unable to reply to review v2");
    }
});
exports.publishMaturedReviewsV2 = (0, scheduler_1.onSchedule)({ region: env_1.PROJECT_REGION, schedule: "every 24 hours" }, async () => {
    const now = firebase_admin_1.default.firestore.Timestamp.now();
    const snap = await firestore_1.db.collection(REVIEWS_COLLECTION).where("status", "==", "pending_peer_review").limit(200).get();
    const batch = firestore_1.db.batch();
    ...
    for (const doc of snap.docs) {
        ...
        if (!(publishAfter instanceof firebase_admin_1.default.firestore.Timestamp) || publishAfter.toMillis() > now.toMillis())
            continue;
        batch.set(doc.ref, { status: "published", visibleOnProfile: true, publishedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(), updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp() }, { merge: true });
    ...
    logger_1.logger.info("marketplace_reviews_v2_matured", { count });
});
//# sourceMappingURL=reviews_v2.js.map