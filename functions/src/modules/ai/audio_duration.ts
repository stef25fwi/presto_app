...
export type AudioContainer = "wav" | "mp3" | "ogg" | "flac" | "mp4" | "webm";
...
function readUInt24BE(buffer: Buffer, offset: number): number | null {
  if (offset + 3 > buffer.length) return null;
  return (
    (byteAt(buffer, offset) << 16) |
    (byteAt(buffer, offset + 1) << 8) |
    byteAt(buffer, offset + 2)
  );
}

function round(seconds: number): number | null {
...
export function sniffAudioContainer(buffer: Buffer): AudioContainer | null {
  ...
  if (byteAt(buffer, 0) === 0xff && (byteAt(buffer, 1) & 0xe0) === 0xe0) return "mp3";
  return null;
}

/** Déduit le conteneur du content-type lorsque les octets ne suffisent pas. */
export function containerFromContentType(
  contentType: string,
): AudioContainer | null {
  ...
  if (value.includes("webm") || value.includes("matroska")) return "webm";
...
export function estimateWavDuration(buffer: Buffer): number | null {
...
export function estimateFlacDuration(buffer: Buffer): number | null {
  if (buffer.toString("ascii", 0, 4) !== "fLaC") return null;
  let offset = 4;
  while (offset + 4 <= buffer.length) {
    ...
    const body = offset + 4;
    if (type === 0) {
      if (body + 18 > buffer.length) return null;
      const sampleRate =
        (byteAt(buffer, body + 10) << 12) |
        (byteAt(buffer, body + 11) << 4) |
        (byteAt(buffer, body + 12) >> 4);
      // 36 bits : 4 bits de poids fort puis 4 octets.
      const totalSamples =
        (byteAt(buffer, body + 13) & 0x0f) * 2 ** 32 + buffer.readUInt32BE(body + 14);
...
function readOggPage(buffer: Buffer, offset: number): OggPage | null {
  if (offset + 27 > buffer.length) return null;
  if (buffer.toString("ascii", offset, offset + 4) !== "OggS") return null;
  ...
  if (tableStart + segmentCount > buffer.length) return null;
  let payloadLength = 0;
...
export function estimateOggDuration(buffer: Buffer): number | null {
  const first = readOggPage(buffer, 0);
  if (!first) return null;
  const payload = buffer.subarray(
    first.payloadStart,
    first.payloadStart + first.payloadLength,
  );
  ...
  if (lastPageOffset < 0) return null;
  const last = readOggPage(buffer, lastPageOffset);
  if (!last || last.serial !== first.serial) return null;
  const samples = last.granulePosition - preSkip;
  return round(samples / sampleRate);
}

export function estimateMp4Duration(buffer: Buffer): number | null {
...
function findMp4Atom(
  ...
  while (offset + 8 <= to) {
    ...
    if (name === wanted) {
      if (rest.length === 0) return { start: bodyStart, end: bodyEnd };
...
function parseMp3Frame(buffer: Buffer, offset: number): Mp3Frame | null {
  if (offset + 4 > buffer.length) return null;
  if (byteAt(buffer, offset) !== 0xff) return null;
  if ((byteAt(buffer, offset + 1) & 0xe0) !== 0xe0) return null;
  const versionBits = (byteAt(buffer, offset + 1) >> 3) & 0x03;
  const layerBits = (byteAt(buffer, offset + 1) >> 1) & 0x03;
  if (versionBits === 1 || layerBits !== 0x01) return null; // réservé ou hors Layer III
...
function findFirstMp3Frame(buffer: Buffer, from: number): number {
  const limit = Math.min(buffer.length - 4, from + 200_000);
  for (let offset = from; offset <= limit; offset += 1) {
    if (parseMp3Frame(buffer, offset)) return offset;
  }
  return -1;
}

function readMp3VbrFrameCount(
  buffer: Buffer,
  frameStart: number,
  frame: Mp3Frame,
): number | null {
  const sideInfoSize =
    frame.mpegVersion === 1 ? (frame.mono ? 17 : 32) : frame.mono ? 9 : 17;
...
export function estimateMp3Duration(buffer: Buffer): number | null {
...
function readVint(buffer: Buffer, offset: number, stripMarker: boolean): Vint | null {
...
function findEbmlChild(
  ...
  while (offset < to) {
    ...
    if (id.value === wantedId) return { start: bodyStart, end: bodyEnd };
    if (bodyEnd <= offset) return null;
    offset = bodyEnd;
  }
  return null;
}

export function estimateWebmDuration(buffer: Buffer): number | null {
  ...
  let duration: number;
  if (width === 4) duration = buffer.readFloatBE(durationElement.start);
  else if (width === 8) duration = buffer.readDoubleBE(durationElement.start);
  else return null;
...
export function estimateAudioDurationSeconds(
  ...
  const container =
    sniffAudioContainer(buffer) || containerFromContentType(contentType);
  try {
    switch (container) {
      case "wav":
