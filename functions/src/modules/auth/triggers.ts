...
function extractFirstName(data: Record<string, unknown> | undefined): string {
...
async function emitUserLifecycleEvent({
  eventId,
  eventName,
  userId,
  dedupeKey,
  ...
  eventId: string;
  eventName: "profile.verified" | "user.account.deletion.requested" | "user.account.deleted";
  userId: string;
  dedupeKey: string;
  occurredAt: number;
...
export const onUserUpdated = onDocumentUpdated(`${COLLECTIONS.users}/{userId}`, async (event) => {
  ...
  if (!beforeVerified && afterVerified) {
    ...
    await emitUserLifecycleEvent({
      ...
      userId,
      dedupeKey: sha256(`profile.verified:${userId}`),
      occurredAt: now,
      payload: {
  ...
  const beforeDeletionRequestedAt = readTimestampMs(
    before.accountDeletionRequestedAt,
    before.account_deletion_requested_at,
    before.deletionRequestedAt,
    before.deletion_requested_at,
  );
  const afterDeletionRequestedAt = readTimestampMs(
    after.accountDeletionRequestedAt,
    after.account_deletion_requested_at,
    after.deletionRequestedAt,
    after.deletion_requested_at,
  );
  if (beforeDeletionRequestedAt <= 0 && afterDeletionRequestedAt > 0) {
    const effectiveAt = readTimestampMs(
      after.accountDeletionEffectiveAt,
      after.account_deletion_effective_at,
      after.deletionEffectiveAt,
      after.deletion_effective_at,
      after.deleteAt,
      after.delete_at,
    ) || afterDeletionRequestedAt;
    ...
    await emitUserLifecycleEvent({
      ...
      userId,
      dedupeKey: sha256(`user.account.deletion.requested:${userId}:${Math.floor(afterDeletionRequestedAt / 1000)}`),
      occurredAt: now,
      payload: {
        recipient_email: recipientEmail,
        firstName,
        deletionDate: new Date(effectiveAt).toLocaleDateString("fr-FR"),
        cancelDeletionUrl: `${APP_BASE_URL}/mon-compte`,
  ...
  if ((beforeStatus !== "deleted" && afterStatus === "deleted") || (beforeDeletedAt <= 0 && afterDeletedAt > 0)) {
    ...
    await emitUserLifecycleEvent({
      ...
      userId,
      dedupeKey: sha256(`user.account.deleted:${userId}:${Math.floor(deletedAt / 1000)}`),
      occurredAt: deletedAt,
      payload: {
        recipient_email: recipientEmail,
        firstName,
        feedbackUrl: `${APP_BASE_URL}/contact`,
        supportUrl: `${APP_BASE_URL}/support`,
      },
    });
  }
