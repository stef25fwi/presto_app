...
function analyzeReviewText(rawComment: unknown): { comment: string | null; flags: ModerationFlags; status: ReviewStatus } {
  ...
  return {
    ...
    flags,
    status: needsModeration ? "pending_moderation" : "pending_peer_review",
  };
}
...
async function assertReviewEligibility({
  ...
  if (!ownerId) {
    throw new HttpsError("failed-precondition", "Listing owner is missing");
  }

  if (reviewerRole === "requester" && reviewerId !== ownerId) {
    throw new HttpsError("permission-denied", "Only the listing requester can review the provider as requester");
  }
...
function readReviewForScore(doc: admin.firestore.QueryDocumentSnapshot): ReviewForScore | null {
  const data = (doc.data() ?? {}) as Record<string, unknown>;
  const reviewedRole = normalizeString(data.reviewedRole) as ReviewRole;
  const reviewerRole = normalizeString(data.reviewerRole) as ReviewRole;
  if (reviewedRole !== "requester" && reviewedRole !== "provider") return null;
  if (reviewerRole !== "requester" && reviewerRole !== "provider") return null;
  return {
    id: doc.id,
    ...
    reviewerRole,
    reviewedRole,
    criteria: data.criteria && typeof data.criteria === "object"
      ? Object.fromEntries(
        Object.entries(data.criteria as Record<string, unknown>)
          .map(([key, value]) => [key, Number(value || 0)])
          .filter((entry): entry is [string, number] => Number.isFinite(entry[1]) && entry[1] > 0),
      )
      : {},
    averageRating: Number(data.averageRating || data.overallRating || 0),
...
function summarizeRole(reviews: ReviewForScore[], role: ReviewRole) {
  ...
  const reliableAverage = count === 0
    ? 0
    : Number(((PLATFORM_DEFAULT_AVERAGE * BAYESIAN_MIN_REVIEWS + weightedSum) / (BAYESIAN_MIN_REVIEWS + weightTotal)).toFixed(3));
  ...
  const criteriaAverages = Object.fromEntries(
    Array.from(criteriaTotals.entries()).map(([key, value]) => [
      key,
      Number((value.total / value.count).toFixed(3)),
    ]),
  );
...
function summarizeTrustScoreV2(reviews: ReviewForScore[], userData: Record<string, unknown>) {
  ...
  const weightedRoleScore = Math.round(((provider.score100 * 0.65) + (requester.score100 * 0.35)) + profileCompletenessBonus);
  const score100 = Math.max(0, Math.min(100, weightedRoleScore));
  ...
  return {
    ...
    global: {
      ...
      reliableAverage: totalReviews === 0
        ? 0
        : Number(((provider.reliableAverage * provider.reviewsCount + requester.reliableAverage * requester.reviewsCount) / totalReviews).toFixed(3)),
...
function legacyTrustScoreFromV2(scoreV2: ReturnType<typeof summarizeTrustScoreV2>) {
  const allCriteria = {
    ...scoreV2.provider.criteriaAverages,
    ...scoreV2.requester.criteriaAverages,
  } as Record<string, number>;
  const reviewsCount = scoreV2.global.reviewsCount;
  const badges = Array.from(new Set([
    ...scoreV2.provider.badges,
    ...scoreV2.requester.badges,
  ]));
  return {
...
async function recalculateUserTrustScoreV2(userId: string): Promise<Record<string, unknown>> {
  ...
  for (const doc of reviewsSnap.docs) {
    const data = (doc.data() ?? {}) as Record<string, unknown>;
    if (normalizeString(data.status) === "pending_peer_review" || normalizeString(data.status) === "pending_moderation") {
      pendingReviewsCount += 1;
    }
    if (!isReviewPublicForScore(data, nowMs)) continue;
    const review = readReviewForScore(doc);
    if (review) reviews.push(review);
  }

  const userData = (userSnap.data() ?? {}) as Record<string, unknown>;
  const trustScoreV2 = summarizeTrustScoreV2(reviews, userData);
  const trustScore = {
    ...legacyTrustScoreFromV2(trustScoreV2),
    pendingReviewsCount,
  };
...
async function publishReciprocalReviewsIfReady(offerId: string, reviewerId: string, reviewedUserId: string): Promise<void> {
  const counterpartSnap = await db.collection(REVIEWS_COLLECTION)
  ...
  const reviewIds = counterpartSnap.docs
    .filter((doc) => normalizeString(doc.data().status) === "pending_peer_review")
    .map((doc) => doc.id);
  if (reviewIds.length === 0) return;

  const currentSnap = await db.collection(REVIEWS_COLLECTION)
    .where("offerId", "==", offerId)
  ...
  for (const reviewId of Array.from(new Set(reviewIds))) {
    batch.set(db.collection(REVIEWS_COLLECTION).doc(reviewId), {
      status: "published",
...
export const submitMutualVerifiedReview = onCall({ region: PROJECT_REGION, enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
  ...
  const reviewerRole = request.data?.reviewerRole == null
    ? "requester"
    : assertRole(request.data?.reviewerRole, "reviewerRole");
  const reviewedRole = request.data?.reviewedRole == null
    ? oppositeRole(reviewerRole)
    : assertRole(request.data?.reviewedRole, "reviewedRole");
  ...
  try {
    ...
    const [reviewedScore, reviewerScore] = await Promise.all([
      recalculateUserTrustScoreV2(reviewedUserId),
      recalculateUserTrustScoreV2(reviewerId),
    ]);

    await createInAppNotification({
      notificationId: `mutual_review_${reviewId}`,
      userId: reviewedUserId,
      title: moderation.status === "pending_moderation"
        ? "Un avis vous concernant est en cours de vérification."
        : "Vous avez reçu un nouvel avis vérifié sur iliprestō.",
      message: moderation.status === "pending_peer_review"
        ? "Il sera publié quand l’autre avis sera reçu ou à la fin du délai de notation."
    ...
    logger.info("marketplace_mutual_review_submitted", {
      offerId,
      reviewId,
      reviewerId,
      reviewedUserId,
...
export const getUserTrustScoreV2 = onCall({ region: PROJECT_REGION, enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
  ...
  try {
    ...
    const latestReviews = reviewsSnap.docs
      .filter((doc) => isReviewPublicForScore(doc.data() ?? {}, nowMs))
      ...
      .map((review) => ({
        ...
        roleLabel: roleLabel(review.reviewedRole),
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (reviewedUserId !== reporterId && reviewerId !== reporterId) {
        throw new HttpsError("permission-denied", "Only review participants can report this review");
      }
...
export const replyToReviewV2 = onCall({ region: PROJECT_REGION, enforceAppCheck: ENFORCE_APP_CHECK }, async (request) => {
  ...
  try {
    ...
    await replyRef.set({
      ...
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      status: moderation.status === "pending_peer_review" ? "published" : moderation.status,
      moderationFlags: moderation.flags,
      ratingSchemaVersion: 2,
    }, { merge: true });
    ...
    await db.collection(REVIEWS_COLLECTION).doc(reviewId).set({
      hasReply: true,
      replyText: moderation.comment,
      replyStatus: moderation.status === "pending_peer_review" ? "published" : moderation.status,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    }, { merge: true });

    return { ok: true, status: moderation.status === "pending_peer_review" ? "published" : moderation.status };
  } catch (error) {
...
export const publishMaturedReviewsV2 = onSchedule({ region: PROJECT_REGION, schedule: "every 24 hours" }, async () => {
  ...
  for (const doc of snap.docs) {
    ...
    if (!(publishAfter instanceof admin.firestore.Timestamp) || publishAfter.toMillis() > now.toMillis()) {
      continue;
    }
    batch.set(doc.ref, {
      status: "published",
