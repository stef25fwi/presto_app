...
function regionFromDepartment(department: string): string {
  const dept = normalizeString(department).toUpperCase();
  if (!dept) return "";
  for (const [regionCode, departments] of Object.entries(REGION_DEPARTMENTS)) {
...
function buildCityId(city: string, postalCode: string): string {
  const safeCity = normalizeString(city);
  const safePostalCode = normalizePostalCode(postalCode);
  if (!safeCity || !safePostalCode) return "";
...
export function validateListingDraftPayload(rawDraft: UnknownRecord, maxMediaCount: number): ValidatedListingDraftPayload {
  ...
  const rawRegion = normalizeString(rawDraft.region) || regionFromDepartment(rawDept);
