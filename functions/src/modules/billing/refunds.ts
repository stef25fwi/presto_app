...
export const REFUNDS_COLLECTION = "billing_refunds";
export const DISPUTES_COLLECTION = "billing_disputes";
 ...
 */
export function normalizedRefundStatus(
  eventType: string,
  refund: JsonMap,
...
export function disputeOutcome(
  eventType: string,
  dispute: JsonMap,
): "open" | "won" | "lost" | "warning" {
  const status = asString(dispute.status).toLowerCase();
  if (status.startsWith("warning")) return "warning";
  if (status === "won") return "won";
  if (status === "lost") return "lost";

  if (eventType === "charge.dispute.funds_reinstated") return "won";
  if (eventType === "charge.dispute.funds_withdrawn") return "lost";
 ...
 */
export function requiresBillingReview(
  kind: "refund" | "dispute",
  outcome: string,
): boolean {
  if (kind === "dispute") return ["open", "lost", "warning"].includes(outcome);
...
export async function syncRefund(
  ...
  await db.collection(REFUNDS_COLLECTION).doc(refundId).set(data, { merge: true });
...
export async function syncDispute(
  ...
  await db.collection(DISPUTES_COLLECTION).doc(disputeId).set(data, { merge: true });
  await flagUserBilling(context.userId, {
    kind: "dispute",
    reference: disputeId,
    state: outcome,
...
async function flagUserBilling(
  userId: string,
  incident: {
    kind: "refund" | "dispute";
    reference: string;
