...
export function getTemplateMeta(templateCode: string): TemplateRegistryItem | null {
  return templateRegistry.find((item) => item.template_code === templateCode) || null;
}

export function listMissingRequiredVariables(templateCode: string, payload: Record<string, unknown>): string[] {
...
function wrapDefaultHtml(subject: string, preheader: string, body: string): string {
  return (
...
function wrapDefaultText(subject: string, preheader: string, body: string): string {
  return `PRESTO\n\n${subject}\n\n${preheader}\n\n${body}\n`;
}

export function getDefaultTemplateContent(templateCode: TemplateCode, locale: "fr" | "en"): DefaultTemplateContent {
  const meta = getTemplateMeta(templateCode);
  ...
  const frBodies: Partial<Record<TemplateCode, DefaultTemplateContent>> = {
    ...
    tpl_transactional_moderation_listing_report_admin_alert_v1: {
      html: wrapDefaultHtml(
        subject,
        preheader,
        `<p>Bonjour,</p><p>Un nouveau signalement a été reçu pour l'annonce <strong>{{listingTitle}}</strong>.</p><p>Motif: <strong>{{reportReason}}</strong></p><p>Détails: {{reportReasonText}}</p><p>Signalé par: {{reporterName}} ({{reporterEmail}})</p><p><a href="{{listingUrl}}">Voir l'annonce</a> · <a href="{{reportUrl}}">Voir le signalement</a></p>`,
      ),
      text: wrapDefaultText(
        subject,
        preheader,
