...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  /// ANIMATION + DÉCOMPTE CHRONO
  late final AnimationController _chronoPulseController;
  late final Animation<double> _chronoPulseAnimation;
  static const int _chronoInitialSeconds = 59;
  int _chronoRemainingSeconds = _chronoInitialSeconds;
  Timer? _chronoTimer;
  ...
  final List<_HomeSlide> _slides = const [
    _HomeSlide(
      ...
      badge: "Disponible",
      imageAsset: "assets/images/chrono_1min.png", // ⚠️ dans pubspec
    ),
    _HomeSlide(
  ...
  void initState() {
    ...
    // Animation du chrono : effet "pulse"
    _chronoPulseController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 1400),
    )..repeat(reverse: true);

    _chronoPulseAnimation = Tween<double>(begin: 1.0, end: 1.05)
        .chain(CurveTween(curve: Curves.easeInOut))
        .animate(_chronoPulseController);

    // Timer du chrono (boucle 1 minute)
    _startChronoCountdown();
  ...
  void _startChronoCountdown() {
    _chronoTimer?.cancel();
    _chronoRemainingSeconds = _chronoInitialSeconds;

    _chronoTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (!mounted) return;
      setState(() {
        if (_chronoRemainingSeconds <= 0) {
          _chronoRemainingSeconds = _chronoInitialSeconds; // boucle
        } else {
          _chronoRemainingSeconds--;
        }
      });
    });
  }

  String get _chronoText =>
      '00:${_chronoRemainingSeconds.toString().padLeft(2, '0')}';
  ...
  void dispose() {
    ...
    _sloganTimer?.cancel();
    _chronoTimer?.cancel();
    _chronoPulseController.dispose();
    super.dispose();
  }
  ...
  /// -------- SLIDE 0 SPÉCIAL AVEC CHRONO ANIMÉ + GLOSSY --------
  Widget _buildChronoSlide(String animatedText) {
    return Padding(
      ...
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    "Carte des personnes disponibles en quelques secondes.",
                    style: TextStyle(
                      color: Colors.white70,
                      fontSize: 13,
                      fontWeight: FontWeight.w500,
            ...
            // Chrono animé glossy à droite
            Positioned(
              right: -18,
              top: -18,
              child: ScaleTransition(
                scale: _chronoPulseAnimation,
                child: Transform.rotate(
                  angle: -0.08, // léger tilt gauche
                  child: Container(
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(999),
                      boxShadow: const [
                    ...
                    children: [
                      PageView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Slide 0 : version spéciale chrono animé
                          if (index == 0) {
                            return _buildChronoSlide(animatedText);
                          }

                          // Slides suivants : version standard
                          return Padding(
                            padding: const EdgeInsets.symmetric(
...
// On n'affiche "Photos de l’annonce" QUE s'il y a au moins une photo
...
SizedBox(
  width: double.infinity,
  // ✅ Si pas de photos → on utilise une grande bannière
  height: photos.isEmpty ? 190 : 100,
  child: ClipRRect(
...
// (on garde ta bannière 320x100 en dessous)
SizedBox(
  width: double.infinity,
  height: 100,
  child: ClipRRect(
                ...
                Container(
                  ...
                  child: StreamBuilder<
                    ...
                    stream: FirebaseFirestore.instance
                        ...
                        // ⚠️ pas de orderBy Firestore → on trie en Dart
                        .snapshots(),
                    builder: (context, snapshot) {
                      ...
                      // Tri côté client par createdAt (du plus récent au plus ancien)
                      docs.sort((a, b) {
                      ...
                      return Column(
                        children: docs.map((d) {
                          ...
                          final title =
                              (data['title'] ?? 'Sans titre') as String;
                          final location =
                              (data['location'] ?? 'Lieu non précisé')
                                  as String;
                          final createdAt =
