...
class _GuidedJourneyPageState extends State<GuidedJourneyPage> {
  ...
  void _toggleChecklist(String itemId) {
    final done = _checklistDone.putIfAbsent(_activeStage.id, () => <String>{});
    setState(() {
    ...
    try {
      ...
      if (!saveDecision.allowed) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Votre quota de sauvegardes est actuellement atteint.',
            ),
          ),
        );
        return;
      ...
      if (!saveDecision.entitlements.canExportPdf) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Parcours sauvegardé sur cet appareil.'),
          ),
        );
        return;
      }
      ...
      child: Scaffold(
        ...
        body: _showOverview
            ...
            : GuidedJourneyStageView(
                ...
                progress: _progress,
                checkedIds: _checklistDone[_activeStage.id] ?? const <String>{},
