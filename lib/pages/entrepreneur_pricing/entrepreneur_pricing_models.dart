...
class ProductionMachineUsage {
  ...
  double get kwhPerUnit =>
      (math.max(watts, 0) / 1000) *
      (math.max(minutesPerUnit, 0) / 60) *
      math.max(quantity, 0);

  double costPerUnit(double electricityRate) =>
      kwhPerUnit * math.max(electricityRate, 0);
...
class ProductionAccessoryUsage {
  ...
  double get costPerUnit =>
      math.max(quantityPerUnit, 0) * math.max(unitPrice, 0);
...
class EntrepreneurPricingEngine {
  ...
  static EntrepreneurPricingCalculation compute(
    ...
    final materialCost = math.max(draft.materials, 0) +
        math.max(draft.packaging, 0) +
        math.max(draft.consumables, 0);
    final accessoryCost = draft.mode == EntrepreneurPricingMode.expert
        ? draft.accessoriesCostPerUnit
        : 0.0;
    final machineKwh = draft.mode == EntrepreneurPricingMode.expert
        ? draft.machineKwhPerUnit
        : 0.0;
    final machineElectricityCost =
        draft.mode == EntrepreneurPricingMode.expert
            ? draft.machineElectricityCostPerUnit
            : 0.0;
    final waterCost = draft.mode == EntrepreneurPricingMode.expert
        ? math.max(draft.waterM3PerUnit, 0) * math.max(draft.waterRate, 0)
        : 0.0;
    final transportAndOther = draft.mode == EntrepreneurPricingMode.expert
        ? math.max(draft.transportPerUnit, 0) +
            math.max(draft.otherCostsPerUnit, 0)
        : 0.0;
    final laborCost =
        (math.max(draft.workMinutes, 0) / 60) * math.max(draft.hourlyRate, 0);
    final safeVolume = math.max(draft.monthlyVolume, 1);
    final fixedCostPerUnit = math.max(draft.monthlyFixedCosts, 0) / safeVolume;
    final amortization = math.max(draft.equipmentSharePerUnit, 0);
    ...
    final feeRate = (math.max(draft.externalFeePercent, 0) / 100)
        .clamp(0.0, 0.999);
    final fixedFee = math.max(draft.externalFixedFee, 0);
    final marginRate = math.max(draft.marginPercent, 0) / 100;
    final vatRate = math.max(draft.vatPercent, 0) / 100;

    final minimumHt = (costPrice + fixedFee) / (1 - feeRate);
    final suggestedHt = ((costPrice * (1 + marginRate)) + fixedFee) /
        (1 - feeRate);
    final minimumTtc = minimumHt * (1 + vatRate);
    final suggestedTtc = suggestedHt * (1 + vatRate);

    final expectedHt = math.max(draft.expectedPriceTtc, 0) / (1 + vatRate);
    ...
    final unitsToAmortize = draft.equipmentInvestment > 0 && amortization > 0
        ? (draft.equipmentInvestment / amortization).ceil()
        : 0;

    final referenceTtc = draft.expectedPriceTtc > 0
        ? draft.expectedPriceTtc
        : suggestedTtc;
    final referenceHt = referenceTtc / (1 + vatRate);
    ...
    final breakEvenUnits = draft.monthlyFixedCosts <= 0
        ? 0
        : unitContribution > 0
    ...
    return EntrepreneurPricingCalculation(
      ...
      expectedPriceIsProfitable:
          draft.expectedPriceTtc > 0 && expectedProfit >= 0,
    );
  }
  ...
  static EntrepreneurPricingScenario computeScenario(
    ...
    final feeRate = (math.max(draft.externalFeePercent, 0) / 100)
        .clamp(0.0, 0.999);
    final fixedFee = math.max(draft.externalFixedFee, 0);
    final priceTtc = draft.expectedPriceTtc > 0
        ? draft.expectedPriceTtc
        : calculation.suggestedPriceTtc;
    final priceHt = priceTtc / (1 + vatRate);
    final fees = (priceHt * feeRate) + fixedFee;
    final variableCost = calculation.costPrice - calculation.fixedCostPerUnit;
    final monthlyProfit =
        ((priceHt - fees - variableCost) * safeVolume) -
            math.max(draft.monthlyFixedCosts, 0);

    return EntrepreneurPricingScenario(
      label: label,
      volume: safeVolume,
      revenueTtc: priceTtc * safeVolume,
