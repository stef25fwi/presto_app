  ...
  factory GuidedJourneyProgress.fromMap(
    ...
    if (checklistRaw is Map) {
      for (final entry in checklistRaw.entries) {
        checklist['${entry.key}'] =
            (entry.value as List?)?.map((value) => '$value').toSet() ??
            <String>{};
      }
    }
    ...
    return GuidedJourneyProgress(
      activeStageId: stages.any((stage) => stage.id == requested)
          ? requested
          : fallback,
...
class GuidedJourneyContentFactory {
    ...
    if (status.contains('fonctionnaire') || status.contains('agent public')) {
      return const <JourneyChecklistItem>[
        ...
        JourneyChecklistItem(
          ...
          label:
              'Préparer une demande écrite si une autorisation est nécessaire',
        ),
      ];
    }
  ...
  List<String> _mapDetails(List<Map<String, dynamic>> values) =>
      values.map(_describeMapItem).where((item) => item.isNotEmpty).toList();
  ...
  List<JourneyResourceLink> _resources(
    ...
    final normalizedPatterns = patterns
