...
| UI / UX | 🟡 socle sain, accessibilité inachevée | 5 des 8 contrôles restent `pending` (navigation clavier, lecteur d'écran, responsive, cohérence des états, audit final) ; **10 défauts clavier/lecteur d'écran corrigés** sur 6 écrans principaux (15 et 16/08, §2.2) ; responsive et passe appareil réel restent hors d'atteinte du code |
...
|---|---|---|
| Navigation clavier et focus | `pending` | Parcours manuel clavier sur publication / messagerie / compte, non consigné. **Revue de code faite les 15 et 16/08** (`docs/evidence/ux/accessibility-audit.md` §3bis), 6 écrans passés en revue en entier : **10 défauts corrigés** (carte d'accueil, titre AppBar messagerie, suppression pièce jointe, aperçu photo, bulle de message par appui long, en-tête annonceur, révélation téléphone, sélecteur de mode IA, bouton micro, bouton « Remplir avec l'IA », sélecteur de région) + **1 doublon supprimé** (avatar de conversation dupliquant le tap de sa ligne). 27 occurrences restantes, non prioritaires (admin, pages secondaires) |
...
| 4 | Dérouler les contrôles d'accessibilité restants (clavier, lecteur d'écran, responsive, états) | UI/UX | **partiel, fait les 15 et 16/08** — 10 défauts corrigés + 1 doublon supprimé sur 6 écrans principaux, revue des états sur 4/9 parcours (§2.2) ; le clavier/lecteur d'écran sur appareil réel et la matrice responsive restent à faire, élevé |
