# FICHE MÉTIER — Gardiennage maison (absence) — fonctionnaire

## 0. Identité de la fiche

Métier / activité : Gardiennage maison (absence)
...
Cette fiche sert de base de données pour alimenter automatiquement un parcours “Je me lance” lorsqu’un utilisateur sélectionne le statut **Fonctionnaire** et l’activité **Gardiennage maison (absence)**. Le code APE reste indicatif : il est attribué ou confirmé selon l’activité réellement déclarée.
...
Pour **Gardiennage maison (absence)**, l’application doit donc afficher :
...
Coûts indicatifs pour **Gardiennage maison (absence)** :
...
1. Décrire précisément l’activité Gardiennage maison (absence).
...
id_fiche : fonctionnaire_gardiennage_maison_absence
titre : Gardiennage maison (absence) — fonctionnaire
statut_utilisateur : fonctionnaire
categorie : Aide à domicile
activite : Gardiennage maison (absence)
