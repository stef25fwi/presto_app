# Architecture iliprestō

Point d’entrée de la documentation technique :

- [Flux techniques critiques](data-flows.md)
- [Architecture de sécurité](security.md)
- [Modèle Firestore et conventions](firestore-model.md)
- [Architecture Flutter](flutter-architecture.md)
- [Architecture backend](backend-architecture.md)

Compléments :

- [Catalogue des Cloud Functions](../api/cloud-functions.md)
- [Sauvegarde et restauration](../operations/backup-restore.md)
- [Observabilité production](../operations/observability.md)
- [Décisions d’architecture](../adr/README.md)

Toute évolution sensible doit mettre à jour la documentation concernée dans la même pull request.
